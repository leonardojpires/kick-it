                        ...
                        <div class="col-12 col-sm-6 col-md-4">
                            <div class="card shadow-sm h-100">
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <p class="card-text text-muted mb-2 player-count" data-id="{{ $room->id }}">
                                        <strong>Players:</strong> {{ $room->playersWithoutCreator }}/{{ $room->max_users }}
                ...
                fetch("{{ route('rooms.index') }}", {
                    ...
                    .then(data => {
                        data.rooms.forEach((room) => {
                            const playerCounts = document.querySelectorAll(`.player-count[data-id='${room.id}']`);
                            playerCounts.forEach(playerCount => {
                                playerCount.innerHTML = `<strong>Players:</strong> ${room.playersWithoutCreator}/${room.max_users}`;
                            });
                        });
                })
            }, 2000);
